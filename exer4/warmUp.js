...
function toLetterGrade(numGrade) {
  if (numGrade < 0 || numGrade > 100) {
    ...
    return "F"; // grade must be > 0 and < 60, which is an F
  }
}
...
function findLongestWord(string) {
  let words = string.split(" "); // uses space as a separator to separate original string
  let maxLength = 0;
...
function combineObjects(object1, object2) {
  return Object.assign({}, object1, object2); // if keys are the same, object2's key value overrides object1's
}
